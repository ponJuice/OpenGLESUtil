...
public class Enemy extends Bullet {
    ...
    protected boolean item_flag = true;
    ...
    public void setDestroyNum(IDPack n){
        destroy = n;
    }
        ...
        if(isDead()){
            if(item_flag){
                if(item != null) {
                    ...
                    item.vel.setY(0.0098f);
                    Item.add(item);
                }
                if(destroy.sp != null){
                    destroy.sp.play(destroy.id,1f,1f,0,0,1f);
                }
                item_flag = false;
            }
            if(state == DAMAGE_STATE.EFFECT){

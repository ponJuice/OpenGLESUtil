...
public class MenuScreen implements Screenable {
	...
	public class OptionScreen{
		...
		public void draw(){
			if(end){
				if(counter <= 60) {
					GLES20Util.DrawGraph(GLES20Util.getWidth_gl() / 2f, GLES20Util.getHeight_gl() / 2f + (offsetY / 60 * counter), GLES20Util.getWidth(), GLES20Util.getHeight_gl(), black, 0.5f, GLES20COMPOSITIONMODE.ALPHA);
					isDebug.draw(0, offsetY / 60 * counter);
					onOffDraw(0, offsetY / 60 * counter, isDebug);
					back.draw(0, offsetY / 60 * counter);
					counter++;
					return;
				}else{
					os = null;
			...
			if(counter <= 60) {
				GLES20Util.DrawGraph(GLES20Util.getWidth_gl() / 2f, GLES20Util.getHeight_gl() / 2f + (offsetY - (offsetY / 60 * counter)), GLES20Util.getWidth(), GLES20Util.getHeight_gl(), black, 0.5f, GLES20COMPOSITIONMODE.ALPHA);
				isDebug.draw(0, (offsetY - (offsetY / 60 * counter)));
				onOffDraw(0,(offsetY - (offsetY/60*counter)),isDebug);
				back.draw(0,(offsetY - (offsetY/60*counter)));
				counter++;
			}else {
